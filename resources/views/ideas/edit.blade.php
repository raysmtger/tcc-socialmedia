            ...
            <div class="card-body p-4 p-md-5">
                ...
                <div class="mb-4">
                    <h3 class="fw-bold mb-1" style="color: #f29d35;">
                        <i class="bi bi-pencil-square"></i> Editar Ideia
                    </h3>
                    <p class="text-muted small mb-0">Continue desenvolvendo seus pensamentos</p>
                </div>
                ...
                <form action="{{ route('ideas.update', $idea->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="descricao" id="descricao" rows="12"
                                  ...
                                  style="font-size: 1rem; line-height: 1.6;">{{ old('descricao', $idea->descricao) }}</textarea>
                    ...
                    {{-- Imagens existentes --}}
@php
    $imagensExistentes = is_array($idea->imagens_descricao) 
...
@if(!empty($imagensExistentes))
    <div class="mb-3">
        <label class="form-label text-muted small">
            <i class="bi bi-images me-1"></i>Imagens atuais
        </label>
        <div class="row g-2" id="existing-images">
            @foreach($imagensExistentes as $index => $imagem)
                <div class="col-6 col-md-3" id="image-{{ $index }}">
                    <div class="position-relative">
                        <img src="{{ Storage::url($imagem) }}" 
                        ...
                        <button type="button" 
                                ...
                                onclick="deleteExistingImage('{{ $idea->id }}', '{{ $imagem }}', {{ $index }})"
...
@endif
                    ...
                    <div class="mb-4">
                        <label class="form-label text-muted small">
                            <i class="bi bi-image me-1"></i>Adicionar novas imagens de referência
                        </label>
                    ...
                    <div class="border-top pt-4 mt-4">
                        <button class="btn btn-link text-muted p-0 mb-3 text-decoration-none" 
                            ...
                            <i class="bi bi-dash-circle me-2" id="toggleIcon" style="font-size: 1.1rem;"></i>
                            <span>Informações adicionais (opcional)</span>
                        </button>

                        <div class="collapse show" id="detalhesAdicionais">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="prazo" id="prazo"
                                           ...
                                           value="{{ old('prazo', $idea->prazo?->format('Y-m-d')) }}">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="nome_cliente" id="nome_cliente"
                                           ...
                                           value="{{ old('nome_cliente', $idea->nome_cliente) }}">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="plataforma" id="plataforma" class="form-select" style="height: 38px; font-size: 0.95rem;">
                                        ...
                                        <option value="instagram" {{ old('plataforma', $idea->plataforma) == 'instagram' ? 'selected' : '' }}>Instagram</option>
                                        <option value="facebook" {{ old('plataforma', $idea->plataforma) == 'facebook' ? 'selected' : '' }}>Facebook</option>
                                        <option value="tiktok" {{ old('plataforma', $idea->plataforma) == 'tiktok' ? 'selected' : '' }}>TikTok</option>
                                        <option value="linkedin" {{ old('plataforma', $idea->plataforma) == 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                                        <option value="twitter" {{ old('plataforma', $idea->plataforma) == 'twitter' ? 'selected' : '' }}>Twitter/X</option>
                                        <option value="youtube" {{ old('plataforma', $idea->plataforma) == 'youtube' ? 'selected' : '' }}>YouTube</option>
                                        <option value="pinterest" {{ old('plataforma', $idea->plataforma) == 'pinterest' ? 'selected' : '' }}>Pinterest</option>
                                        <option value="outras" {{ old('plataforma', $idea->plataforma) == 'outras' ? 'selected' : '' }}>Outras</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="tipo_conteudo" id="tipo_conteudo" class="form-select" style="height: 38px; font-size: 0.95rem;">
                                        <option value="">Selecione...</option>
                                        <option value="reels" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'reels' ? 'selected' : '' }}>Reels</option>
                                        <option value="carrossel" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'carrossel' ? 'selected' : '' }}>Carrossel</option>
                                        <option value="post_unico" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'post_unico' ? 'selected' : '' }}>Post Único</option>
                                        <option value="stories" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'stories' ? 'selected' : '' }}>Stories</option>
                                        <option value="video" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'video' ? 'selected' : '' }}>Vídeo</option>
                                        <option value="artigo" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'artigo' ? 'selected' : '' }}>Artigo</option>
                                        <option value="thread" {{ old('tipo_conteudo', $idea->tipo_conteudo) == 'thread' ? 'selected' : '' }}>Thread</option>
                                    </select>
                                </div>
                            </div>
                        </div>
    ...
    <script>
       ...
       // deletar imagem
function deleteExistingImage(ideaId, imagePath, index) {
    ...
    fetch(`/ideas/${ideaId}/remove-image`, {
        method: 'DELETE',
        headers: {
